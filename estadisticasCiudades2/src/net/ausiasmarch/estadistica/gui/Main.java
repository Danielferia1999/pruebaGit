...
public class Main extends javax.swing.JFrame {
    ...
    private void initComponents() {
        ...
        jComboBoxComunidad.setModel(new javax.swing.DefaultComboBoxModel<>(new String[] { "Andalucia", "Asturias", "Aragón", "Canarias", "Castilla la Mancha", "Castilla y León", "Cataluña", "Comunidad de Madrid", "Comunidad Valenciana", "Extremadura", "Galicia", "La Rioja", "Navarra", "País Vasco", "Región de Murcia", "Ceuta", "Melilla" }));
    ...
    private void jButtonAgregarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButtonAgregarActionPerformed
        ...
        // Validamos los datos
       if(sCiudad.length() == 0) {
    	   mensaje("El nombre de la ciudad no puede quedar vacio");
       }
        if(!Convert.isValidInt(sHabitantes)) {
     	   mensaje("Los habitantes no puede quedar vacio");
	
        }
           
        // Comprobamos si la ciudad esta la lista
        for (Ciudad c: ciudades) {
        	if(c.getCiudad().compareToIgnoreCase(sCiudad) == 0) {
        ...
        // Creamos la ciudad
        Ciudad sc = new Ciudad(sComunidad, sCiudad, habitantes);
    ...
    private void jButtonBorrarCiudadActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButtonBorrarCiudadActionPerformed
        ...
        if (fila >= 0) {           
            //Borramos la ciudad
        	gestorCiudades.borrarCiudad(nombreCiudad);
    ...
    private void jButtonCalcularActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButtonCalcularActionPerformed
      int nciudad;
      String cmhabitantes;
	long thabitantes, habitantes;
    	// Obtenemos los calculos
    	nciudad = gestorCiudades.aslistaCiudades(ciudades).size();
    	cmhabitantes = gestorCiudades.ciudadMayorHabitantes().getCiudad();
    	thabitantes = gestorCiudades.totalHabitantes();
    	habitantes = gestorCiudades.ciudadMayorHabitantes().getHabitantes();
    	// Mostramos los calculos
    	 jTextFieldNumCiudades.setText(String.valueOf(nciudad));
    	 jTextFieldCiudadMayorHab.setText(cmhabitantes);
    	 jTextFieldMayorHabitantes.setText(String.valueOf(habitantes));
    	 jTextFieldTotal.setText(String.valueOf(thabitantes));
    ...
    private void jButtonBuscarCiudadActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButtonBuscarCiudadActionPerformed
        ...
        // Buscamos la ciudad Obtenemos los habitantes de la ciudad y los mostramos 
    	if(ciudades.isEmpty()) {
    		return;
    	} else {
    		String nombreciudad = jTextFieldBusqueda.getText();
    		  Ciudad ciudad = gestorCiudades.buscarCiudad(nombreciudad);
    		  if (ciudad ==null) {
    			  mensaje("No se encuenta la ciudad");
    		  }
    		  long habitantes = gestorCiudades.buscarCiudad(nombreciudad).getHabitantes();
    		jTextFieldNumHabitantes.setText(String.valueOf(habitantes));
    	}
